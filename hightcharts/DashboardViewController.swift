...
class DashboardViewController: UITableViewController, HIChartViewDelegate,NotifyHandleDelegate {
    
    var sources: [[String:Any]]!
    var data: [Any]!
    var dataName: String!
    var charts: [HIChartView]!
    var mtcpClient:CTCPClient?
    var mdataParse:CDataParse?
    var mmeterdocParse:CMeterDocParse?
    var startStamp:Int=0
    var lockDataOperte:NSLock?
    var serverSearch:Bool = false
    ...
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        ...
        self.lockDataOperte = NSLock()
        
        // temp
        self.mdataParse = CDataParse()
        self.mmeterdocParse = CMeterDocParse(dataHandle: mdataParse!, notifyHandle: self)
        self.mtcpClient = CTCPClient(dataHandle: mmeterdocParse!, notifyHandle: self)
        ...
        // search wifimeter to get data
        let searchwifimeterWorkItem = DispatchWorkItem{
            if !self.serverSearch
            {
                self.serverSearch = CCommonFunc.searchDataService(socket:self.mtcpClient!)
            }
        }
        DispatchQueue.global(qos:.background).asyncAfter(deadline: DispatchTime.now() + 1, execute: searchwifimeterWorkItem)
        
        // background history data handle process
...
//                else
//                {
                    // get start data stamp
//                    if self.startStamp == 0
//                    {
//                        //self.lockDataOperte?.lock()
//                        self.startStamp = CCommonFunc.getStartStamp()
//                        //self.lockDataOperte?.unlock()
//
//                        //self.lockDataOperte?.lock()
//
//                        if CCommonFunc.connectServer(socket:self.mtcpClient!)
//                        {
//                            CCommonFunc.getTerminalData(startstamp: &self.startStamp,socket:self.mtcpClient!)
//                        }
//
//                        //self.lockDataOperte?.unlock()
//                    }
                    ...
                    {
                        gArrayMeter[i].getHistoryData()
                    }
                
                    // handle by time process
                    switch(CCommonFunc.timeProcessFlag())
                    {
                    case .hourFlag:
//                        if CCommonFunc.connectServer(socket:self.mtcpClient!)
//                        {
...
//                        {
//                            CCommonFunc.getTerminalData(startstamp: &self.startStamp,socket:self.mtcpClient!)
//                            CCommonFunc.dayDataStatistics()
//                        }
                        let billingDay = CCommonFunc.getConfigValuesByKey(parent: "数据信息", sub: "结算日") as! Int
                        for i in 0..<gArrayMeter.count
                        {
                            gArrayMeter[i].passdayProcess()
                            
                            if billingDay == NSDate().getDay(){
                                gArrayMeter[i].calcBillingEnengy(billingDay)
                            }
                        }
                    case .monthFlag:
//                        if CCommonFunc.connectServer(socket:self.mtcpClient!)
...
//                            CCommonFunc.getTerminalData(startstamp: &self.startStamp,socket:self.mtcpClient!)
//                            CCommonFunc.dayDataStatistics()
//                            CCommonFunc.monthDataStatistics()
//                        }
                        for i in 0..<gArrayMeter.count
                        {
                            gArrayMeter[i].passmonthProcess()
                        }
                    case .minuteFlag:
                        print("now time:\r\n",NSDate())
                        /*if CCommonFunc.connectServer(socket:self.mtcpClient!)
        ...
        DispatchQueue.global(qos:.background).asyncAfter(deadline: DispatchTime.now() + 2, execute: workItem)
    }
    ...
    private func needGetDataFromTerminal()->Bool
    {
        return true
    }
    ...
    override func viewDidLoad() {
        ...
        /*CCoreDataOperate.deleteData(name: "Voltage", conditions: nil)
        CCoreDataOperate.deleteData(name: "Current", conditions: nil)
        CCoreDataOperate.deleteData(name: "ActivePower", conditions: nil)
        CCoreDataOperate.deleteData(name: "ReactivePower", conditions: nil)
        CCoreDataOperate.deleteData(name: "Factor", conditions: nil)
        CCoreDataOperate.deleteData(name: "Freq", conditions: nil)
        CCoreDataOperate.deleteData(name: "Gauge", conditions: nil)*/
        //CCoreDataOperate.deleteData(name: "Voltage", conditions: nil)
        //CCoreDataOperate.deleteData(name: "Voltage", conditions: nil)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            ...
            let chartView = HIChartView(frame: CGRect(x: 10.0, y: 5.0, width: self.view.bounds.size.width - 20, height: 240))
            ...
            let series = seriesData[self.dataName] as! [Any]
            if(options["chartType"] as! String=="pie")
            {
                sum = 100
            }
            else
    ...
    private func loadSourcesAndData() {
        ...
        // fill wifimeter info
        let meters = CCommonFunc.getTreeStructure(gConfigData,"设备信息", "Childrens") as! [[String:Any]]
        var meterserial:String = "2018010001"
        var meterhostip:String = "192.168.1.101"
        for i in 0..<meters.count
        {
            let meterconfig = meters[i]["Childrens"] as! [[String:Any]]
            for j in 0..<meterconfig.count
            {
                if meterconfig[j]["name"] as! String == "电表编号"{
                   meterserial = meterconfig[j]["value"] as! String
                }
                if meterconfig[j]["name"] as! String == "IP地址"{
                    meterhostip = meterconfig[j]["value"] as! String
                }
            }
            
            //let meterdoc = MeterDoc(1,)
            let wifimeter = CWifiMeter(meter_id: 1, meter_serial: "")
            
            gArrayMeter.append(wifimeter)
        }
        
        let now = NSDate() //.getPrevDay()//timeInterval: 28800, since: NSDate() as Date)
        self.data = CCommonFunc.fillShowData(timeDate:now,typeData:"day",loadSources:self.sources)
    }
